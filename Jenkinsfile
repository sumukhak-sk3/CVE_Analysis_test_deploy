        ...
        script {
          withCredentials([
            ...
            sh '''#!/usr/bin/env bash
              ...
              echo "Checking Python dependencies for S3 polling gate..."
...
cfg = json.loads(Path("${AUTOMATION_CONFIG_PATH_CLEAN}").read_text(encoding="utf-8"))
s3_cfg = cfg.get("s3") or {}

bucket = (s3_cfg.get("bucket_name") or "").strip()
if not bucket:
    raise SystemExit("s3.bucket_name is required in automation config")
