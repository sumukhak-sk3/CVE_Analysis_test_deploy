import { useState } from "react";
import { api } from "../api";

export default function JenkinsPage() {
  const [sbomFile, setSbomFile] = useState<File | null>(null);
  const [jenkinsUrl, setJenkinsUrl] = useState("");
  const [jobName, setJobName] = useState("");
  const [dtrackUrl, setDtrackUrl] = useState("http://localhost:8081");
  const [analysisApiUrl, setAnalysisApiUrl] = useState("http://10.120.23.228:18088");
  const [repoRoot, setRepoRoot] = useState("");
  const [waitMinutes, setWaitMinutes] = useState("30");
  const [severities, setSeverities] = useState("CRITICAL,HIGH");
  const [limit, setLimit] = useState("");
  const [mode, setMode] = useState("standard");
  const [result, setResult] = useState<any>(null);
  const [error, setError] = useState<string | null>(null);
  const [submitting, setSubmitting] = useState(false);

  async function trigger() {
    if (!sbomFile) {
      setError("Select an SBOM file before triggering Jenkins.");
      return;
    }
    setError(null);
    setSubmitting(true);
    try {
      const uploaded = await api.jenkinsUploadSbom(sbomFile);
      const payload: Record<string, unknown> = {
        ticket: uploaded.ticket,
        jenkins_url: jenkinsUrl || undefined,
        job_name: jobName || undefined,
        dtrack_url: dtrackUrl || undefined,
        analysis_api_url: analysisApiUrl || undefined,
        repo_root: repoRoot || undefined,
        wait_minutes: waitMinutes ? Number(waitMinutes) : undefined,
        severities: severities
          .split(",")
          .map((s) => s.trim())
          .filter(Boolean),
        limit: limit ? Number(limit) : undefined,
        mode: mode || "standard",
      };
      const res = await api.jenkinsTrigger(payload);
      setResult({ ...res, upload: uploaded });
    } catch (e) {
      setError(String(e));
    } finally {
      setSubmitting(false);
    }
  }

  return (
    <div>
      <h2>Jenkins</h2>
      <div className="card">
        <p className="muted" style={{ marginTop: 0 }}>
          Upload an SBOM file and trigger a Jenkins job that runs the
          Dependency-Track ingestion and Workflow D CVE analysis pipeline.
        </p>

        <label>SBOM file</label>
        <input
          type="file"
          accept=".yaml,.yml,.json,.xml,text/yaml,application/yaml"
          onChange={(e) => setSbomFile(e.target.files?.[0] ?? null)}
          style={{ width: "100%" }}
        />

        <div className="grid-2" style={{ marginTop: 12 }}>
          <div>
            <label>Jenkins URL (optional override)</label>
            <input
              value={jenkinsUrl}
              onChange={(e) => setJenkinsUrl(e.target.value)}
              placeholder="https://jenkins.example.com"
              style={{ width: "100%" }}
            />
          </div>
          <div>
            <label>Job name (optional override)</label>
            <input
              value={jobName}
              onChange={(e) => setJobName(e.target.value)}
              placeholder="folder/cve-sbom-pipeline"
              style={{ width: "100%" }}
            />
          </div>
          <div>
            <label>Dependency-Track URL</label>
            <input
              value={dtrackUrl}
              onChange={(e) => setDtrackUrl(e.target.value)}
              placeholder="http://localhost:8081"
              style={{ width: "100%" }}
            />
          </div>
          <div>
            <label>Workflow D API URL</label>
            <input
              value={analysisApiUrl}
              onChange={(e) => setAnalysisApiUrl(e.target.value)}
              placeholder="http://10.120.23.228:18088"
              style={{ width: "100%" }}
            />
          </div>
          <div>
            <label>Analysis repo root (optional)</label>
            <input
              value={repoRoot}
              onChange={(e) => setRepoRoot(e.target.value)}
              placeholder="/path/to/repo"
              style={{ width: "100%" }}
            />
          </div>
          <div>
            <label>Wait minutes for DT enrichment</label>
            <input
              value={waitMinutes}
              onChange={(e) => setWaitMinutes(e.target.value)}
              type="number"
              min={1}
              style={{ width: "100%" }}
            />
          </div>
          <div>
            <label>Severities (comma-separated)</label>
            <input
              value={severities}
              onChange={(e) => setSeverities(e.target.value)}
              placeholder="CRITICAL,HIGH"
              style={{ width: "100%" }}
            />
          </div>
          <div>
            <label>Limit (optional)</label>
            <input
              value={limit}
              onChange={(e) => setLimit(e.target.value)}
              type="number"
              min={1}
              style={{ width: "100%" }}
            />
          </div>
          <div>
            <label>Workflow mode</label>
            <select
              value={mode}
              onChange={(e) => setMode(e.target.value)}
              style={{ width: "100%" }}
            >
              <option value="standard">standard</option>
              <option value="urgent">urgent</option>
              <option value="ad_hoc">ad_hoc</option>
            </select>
          </div>
        </div>

        <label style={{ marginTop: 12 }}>Server path fallback (advanced)</label>
        <input
          placeholder="Not needed for normal use. Upload is the default handoff."
          disabled
          style={{ width: "100%" }}
        />

        <div className="row" style={{ marginTop: 12 }}>
          <button
            className="primary"
            onClick={trigger}
            disabled={submitting || !sbomFile}
          >
            {submitting ? "Submitting…" : "Trigger Jenkins Job"}
          </button>
        </div>
        {error && <div className="error" style={{ marginTop: 12 }}>{error}</div>}
        {result && (
          <div className="success-banner" style={{ marginTop: 12 }}>
            Queued ticket <code>{result.ticket}</code> · state {result.state}
            {result.queue_url ? (
              <>
                {" "}
                · queue <a href={result.queue_url} target="_blank" rel="noreferrer">open</a>
              </>
            ) : null}
          </div>
        )}
      </div>
    </div>
  );
}
